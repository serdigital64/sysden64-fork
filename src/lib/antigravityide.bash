...
function module_antigravityide_setup() {
  bl64_dbg_app_show_function "$@"
  local home="$1"
  local module_type="$SYSDEN64_MODULE_TYPE_SHARED"
  local module_name='antigravityide'
  local module_etc=''
  local extra_locations=''

  [[ "$BL64_OS_TYPE" != "$BL64_OS_TYPE_LINUX" ]] && return 0
  module_detect "$module_name" 'antigravity-ide' 'Antigravity IDE - AI IDE' "$extra_locations" || return 0

  module_etc="$(module_config_get_source "$module_type" "$module_name")" &&
    module_antigravityide_setup_config "$home" "$module_etc" "$module_name" "$module_type"
}

function module_antigravityide_setup_config() {
  bl64_dbg_app_show_function "$@"
  local home="$1"
  local module_etc="$2"
  local module_name="$3"
  ...
  if [[ "$BL64_OS_TYPE" == "$BL64_OS_TYPE_LINUX" ]]; then
    config_destination="${home}/.config/Antigravity IDE"
    bl64_fs_dir_create \
